...
(function (factory) {
	...
	var Tree = function (element, options) {
		...
		this.options.iconClassArr = ['red','pink','orange','blue','green',''];
		this.options.iconClassIndex = 0;
		...
		if( this.options.folderSelect ){
			...
			this.$element.on('click.fu.tree', '.tree-branch-header', $.proxy( function(ev) { this.selectFolder($(ev.currentTarget)); return false;}, this));
		}
	...
	Tree.prototype = {
				...
				$.each( items.data, function(index, value) {
					...
					if(value.type === 'folder') {
						...
						if('icon-class' in value) {
							...
							var iconClassArr = self.options.iconClassArr;
							var iconClass = iconClassArr[parseInt(Math.random() * iconClassArr.length)];
							$entity.attr('iconClass', iconClass);
							header.find('i').addClass(iconClass);
						}

						if('additionalParameters' in value && 'children' in value.additionalParameters){
							var $treeFolderContent = $entity.find('.tree-branch-children');
							$treeFolderContent.addClass('hide');
							self.populate($treeFolderContent);
						}

						var itemExpand = $entity.attr('data-expand');
						
						if(!itemExpand && ('additionalParameters' in value
								&& 'item-expand' in value.additionalParameters 
									&& value.additionalParameters['item-expand'] == true)) {
								itemExpand = true;
						}
						...
						if(('additionalParameters' in value
							&& 'item-selected' in value.additionalParameters 
								...
								$parent.attr('data-expand', true);
								setTimeout(function(){header.trigger('click')}, 0);
						}

						if(itemExpand){
								$parent.attr('data-expand', true);
								setTimeout(function(){$('.icon-folder', header).trigger('click')}, 0);
						}
						...
						var iconClass = $parent.attr('iconClass');
						$entity.find('.tree-item-name > .tree-label > .ace-icon').addClass(iconClass);
						...
						if('additionalParameters' in value
							&& 'item-selected' in value.additionalParameters 
								...
								setTimeout(function(){$entity.trigger('click')}, 0);
		...
		selectFolder: function (clickedElement) {
			...
			var $selectedBranch = this.$element.find('.tree-branch-header.tree-selected, .tree-item.tree-selected');
			var clickedData = $clickedBranch.data();
			var selectedData = [];
			var eventType = 'selected';

			// select clicked item
			var $clickedBranceHead = $('.tree-branch-header', $clickedBranch).eq(0);
			if($clickedBranceHead.hasClass('tree-selected')) {
				// eventType = 'deselected';
				// $clickedBranceHead.removeClass('tree-selected');
			} else {
				$clickedBranceHead.addClass('tree-selected');
			}

			if (this.options.multiSelect) {

			// get currently selected
				$selectedBranch = this.$element.find('.tree-branch-header.tree-selected, .tree-item.tree-selected');
