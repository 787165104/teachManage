...
var IMG_AJAXPATH = "upload";//异步传输服务端位置
...
function previewImg(thi){
	var length=$(thi).parent().find("img").length;
 	
	   if (length >= IMG_MAXCOUNT) {
	        alert("一次最多上传" + IMG_MAXCOUNT + "张图片");
	        return;
	    }
...
$(".imgfile").live("change", function () {
    if ($(this).val().length > 0) {//判断是否有选中图片
       ...
       // IMG_PROGRESS.setAttribute("class", "lookimg_progress");
        ...
        alert( $(this).next()[0].innerHTML);
        $($(this).next()[0]).children("div:last").before(_prevdiv);
...
$("#btn_ImgUpStart").click(function () {
    ...
    for (var i = 0; i < $(".lookimg").length; i++) {
        ...
        //上传图片准备
      //  uploadfile.fileBase64 = $.base64.encode(e.target.result);
